...
ModbusSlaveRTU::ModbusSlaveRTU(HardwareSerial *port, uint32_t baud, uint8_t slaveID, uint16_t registersAddress, uint16_t *registers, uint16_t registersSize, uint64_t timeout)
{
...
void ModbusSlaveRTU::setREDE(uint8_t pinREDE)
{
...
uint8_t ModbusSlaveRTU::Update(void)
{
	if((*port).available())
	{
		lastTimeout = millis();
...
uint16_t ModbusSlaveRTU::conversionToUint16(uint32_t variable, bool bigEndian)
{
	if(bigEndian)
	{
		return (variable >> 16);
...
uint32_t ModbusSlaveRTU::conversionToUint32(uint16_t variable0, uint16_t variable1, bool bigEndian)
{
	if(bigEndian)
	{
...
float ModbusSlaveRTU::conversionToFloat(uint32_t variable)
{
	return *(float*)&variable;
}

void ModbusSlaveRTU::sendAnswer(uint8_t length)
{	
	if(pinREDE != -1)
	{
...
void ModbusSlaveRTU::sendException(uint8_t function, uint8_t exception)
{
	frame[0] = slaveID;
