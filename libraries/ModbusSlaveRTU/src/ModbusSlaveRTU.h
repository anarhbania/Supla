#ifndef MODBUS_SLAVE_RTU_H
#define MODBUS_SLAVE_RTU_H
...
class ModbusSlaveRTU
{
	...
	ModbusSlaveRTU(HardwareSerial *port, uint32_t baud, uint8_t slaveID, uint16_t registersAddress, uint16_t *registers, uint16_t registersSize, uint64_t timeout);
	uint8_t Update(void);
